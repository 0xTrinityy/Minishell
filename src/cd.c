...
void	change_env_pwd(char **env, char *str, t_pars *pars)
{
	char	new_pwd[4096];
	int		j;
	int		i;
...
void	change_env_oldpwd(char **env, char *old_pwd, char *str, t_pars *pars)
{
	int	j;
	int	i;
...
void	ft_cd(t_pars *pars, char **env)
{
	...
	if (!change_dir(path))
		return ;
	change_env_pwd(env, "PWD=", pars);
	change_env_oldpwd(env, old_pwd, "OLDPWD=", pars);
	g_global = 0;
}
